...
| Finance | `finance/finance.js` (bridge, claims, collections, projectMoney), `finance/vat.js` (the one VAT rule), `finance/recognition.js` + `revenue-period.js` (explicit/event-backed periods; no insertion-date fallback), `finance/revenue-review.js` (scoped read-only diagnostics), `finance/expenses.js`. |
...
- `nav.js` — NAV_ITEMS, DETAIL_ACCESS, `pageAllowed()`; re-exports PAGE_ACCESS. `routes.js` — `/login` + OTP web flow, `landingFor()`, `/app/:page` dispatcher against the PAGES map, detail routes (`/app/{project,opportunity,client,event,person}/:id`; person redirects to the canonical employee resource profile when linked), sandboxed mail preview.
...
- Showcase lane: `showcase-up.mjs` (a disposable local instance seeded with fully fictional data via `lib/seed-showcase.mjs`) → `showcase-capture.mjs` (49 screenshots) → `showcase-build.mjs` (`npm run showcase`: one self-contained offline HTML file in `showcase/dist/`, gitignored) → `showcase-verify.mjs` (`npm run showcase:verify`). Presentation material only — not part of the app.
...
5. **Retired internal progress-claim service (not exposed to product writes)** (`src/modules/finance/finance.js` `createProgressClaim`): generates an ISSUED invoice (`kind='progress_claim'`, code `<contract>-C<n>`, due +30d) from DELIVERED/ACCEPTED deliverables of the contract's own project; writes `invoice_line` rows and stamps `invoiced_at`. Bad explicit selections are rejected wholesale with named reasons — never silently dropped.
6. **Retired internal collection service (not exposed to product writes)** (`recordCollection`): gross amount validated against outstanding (amount − retention − collected), split net/vat via `splitGross`, invoice flips PAID/PARTIALLY_PAID, `collected_at` stamped on deliverables when the invoice fully settles.
...
- **Additive personal grants**: `user_department_grant` (migration `025`, rebuilt in place by `049`; `src/modules/identity/grants.js`) — closed `GRANTABLE` list (opportunity and project read/create/update, events-manage pairs) granted as Arabic **bundles** (`GRANT_BUNDLES`) at a **level**: one department, a whole sector (all its departments plus department-less rows) or the company (events only), with an optional last day read at request time (ADR-0025). Granter needs an effective scope that reaches the level, `can()` on the resource at that target, and `ownsEmployee` on the target person; self-grant forbidden even for admin; loaded per-request. The «صلاحياته» card on the person page and the `sanad_*_grant` chat tools are both fed by `grantableBundleOptions`, so what is offered is exactly what the save accepts.
...
- **Page policy = menu = 403**: `PAGE_ACCESS` (`src/core/policy/pages.js`) consumed by `pageAllowed()` (`src/web/nav.js`) for both menu rendering and the route guard. `seesCompanyPerformance()` requires **both** company-wide grant width (`effectiveScope` on report|kpi) **and** `user.scope === 'company'`. Finance and contract views are deleted by owner decision (ADR-0017); old URLs return authenticated 410 tombstones and both finance POST endpoints return 410. The timesheet page stays hard-off. Historical finance reads and shared money calculations remain; this is not a data deletion.
...
- `team/person-link.js` resolves account→employee in either linkage direction and enforces the resource gate before redirecting. Unlinked accounts retain the existing gated dossier. No fuzzy identity matching.
- The canonical employee profile adds a management tab through shared `personActions`; staffing opens the existing period/capacity/approval workflow, while quick tasks and grants keep their original services.
- A deliverable's recognition period is explicit year+month, otherwise a valid accepted/delivered/status event. Neither sale year nor insertion date supplies the period. Conflicting partial periods are rejected. Historical migration 020 is immutable; this release performs no backfill.
- Manual revenue imports keep gross/net/VAT and period/sector/project consistent through preview→apply→undo. Derived rows must be edited at source; nonstandard stored tax treatments cannot be repriced through the standard gross importer. Undo rejects later changed rows.
