...
test('scenarios: كل عيب مثبَّت سابقاً عولج — ولا يعود التثبيت بلا انتباه', () => {
  // التثبيت كان يقول «هذا خطأ نعرفه ولم نُصلحه». كانت تسعةً حين كُتبت الحزمة، وعولجت كلها
  // قبل التفعيل لكل الموظفين — وكل واحد تحوّل تثبيتُه إلى فحصٍ دائم على **الصواب** داخل
  // السيناريو نفسه، لا إلى حذفٍ صامت.
  //
  // والصفر هنا ليس هدفاً يُحرَس بل حالةٌ راهنة: من يجد عيباً جديداً يثبّته بـ`t.defect` بلا
  // تردّد — التثبيت أمانةٌ لا عار. الفحص أدناه لا يمنع ذلك؛ يمنع أن يتغيّر سلوكُ عيبٍ مثبَّت
  // فيمرّ التغيير صامتاً.
  assert.equal(out.recorder.defects.length, 0,
    `عيوب مثبَّتة قائمة: ${out.recorder.defects.map((d) => d.id).join('، ')} — إن كانت جديدة فهذا صحيح، وحدِّث هذا الفحص بعددها وسببها`);
