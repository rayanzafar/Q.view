...
export const ROLE_GRANTS = {
  ...
  ceo_office: [
    ...
    // ── سطور قائمة الدخل (ترحيلة ٠٥٠) ──
    // ما تُقفله المالية شهرياً بسطورها. ولا دور مالية في الشركة (قرار مالك)، فالسلطة المالية
    // على مستوى الشركة هنا كما ورثت الفاتورة والتحصيل والمصروف. والحذف ليس منها: الصفّ يُصحَّح
    // بقيمته الجديدة ويرتفع إصداره، ولا يُمحى رقمٌ أُقفل عليه شهر.
    ...crud(['pl_line'], 'company', ['read', 'create', 'update', 'export']),
  ],
  ...
  sector_lead: [
    ...
    // ── سطور قائمة الدخل لقطاعه (ترحيلة ٠٥٠) ──
    // قرار المالك: قائد القطاع يرفع أرقام قطاعه بنفسه. وهي أرقامٌ يقرؤها أصلاً (بوابتا التكلفة
    // والهامش مفتوحتان له أعلاه)، فالرفع امتدادٌ لعمله لا سلطة جديدة — والنطاق «قطاع» يقصره
    // على قطاعه وحده. وبابُ التكلفة شرطٌ ثانٍ على الكتابة تفرضه الخدمة: من لا يرى الرقم لا
    // يكتبه. وأمّا حقلُ المبلغ فلا يُدرج في `SENSITIVE_FIELDS`: الجدول يحمل سطر الإيراد مع
    // سطور الكلفة، وقاعدةُ حقلٍ واحدة كانت ستحجب الإيراد عمّن يملكه — فالحجب بالسطر في
    // `modules/finance/pl-lines.js` لا بالعمود.
    ...crud(['pl_line'], 'sector', ['read', 'create', 'update', 'export']),
  ],
  ...
  // ويعدّل **كل** فرص قطاعه، فأي مدير تطوير أعمال يفتح فرص زملائه ويقلّب أرقامها — والفرصة
  // قبل ترسيتها سرُّ صاحبها. فانقلبت الرؤية: يرى فرصه التي يملكها وحدها، ومدير الإدارة يرى
  // فرص أهل إدارته، وقائد القطاع يبقى على قطاعه. والمنح الثلاثة هنا مقصودة حرفاً:
  //   • **القراءة والتعديل بنطاق «خاصتي»**: صفّه هو (مالكاً أو منشئاً أو مسكَّناً عبر
  //     `opportunityIds`) — لا صفّ زميله ولو في قطاعه نفسه.
  //   • **الإنشاء بنطاق «القطاع» لا «خاصتي»**: فحص الإنشاء يقع على صفٍّ لم يُكتب بعد، وصفٌّ
  //     لم يُكتب لا مالك له — فمنحُ إنشاءٍ «خاصتي» لا يجتاز فحص الهدف أبداً ويُقفل الباب الذي
  //     أراد المالك إبقاءه مفتوحاً. الثنائي الصحيح: يُنشئ في قطاعه، ويقرأ ما صار له.
  //
  // وما تحت الفرصة (العميل وجهة الاتصال والعرض والتسعير والخدمة) يبقى بنطاق القطاع: العميل
  // ليس سرّ فرصة بعينها بل سجلّ علاقةٍ يتشارك البيعُ كلُّه العملَ عليه — وقصُّه يجعل كل مدير
  // تطوير أعمال يعيد إدخال عميلٍ قائم لأن سجلّه محجوب عنه.
  //
  // وقصةُ «الناس» الأقدم باقية بلا مساس: كان يقود فرصاً ولا يرى إنساناً واحداً — لا منح
  // «موظف» ولا «مهمة»، فتنغلق عليه شاشة «الفريق» وعدسة الفريق في «مهامي» وصفحةُ أي شخص،
  // وهو يسكّن الناس على فرصه فعلاً — أي أنه **يسكّن من لا يراه**. فقراءة أهل قطاعه ومهامهم
  // بقيت «قطاعاً» (بخلاف رئيس تطوير الأعمال أعلاه ونطاقه شركي)، و**قراءةً فقط**: الكتابة على
  // الناس تبقى عند مدير الإدارة وقائد القطاع والموارد البشرية.
  ...
  bd_manager: [
    ...
    ...crud(['client', 'contact', 'proposal', 'pricing_line', 'service'], 'sector',
      ['read', 'create', 'update']),
    // قرار المالك ٢٠٢٦-٠٨-١٧، أُعيد وثُبّت ٢٠٢٦-٠٨-٢٣ (ADR-0010): مدير تطوير الأعمال يدير
    // مشاريع قطاعه إدارةً كاملة — إنشاءً وتعديلاً (الحالة والتواريخ ومدير المشروع والقيم
    // والتسكين والمستندات)، لا قراءةً فقط — **خلافاً للوثيقة المرجعية** (§05) بإقرار المالك.
    // بلا **حذف**: إزالة مشروع قائم قرار لا رجعة فيه يبقى لقائد القطاع ولمدير النظام.
    // (الهامش والكلفة يبقيان محجوبين عنه بمصفوفة الحقول الحساسة — المنح لا يفتحهما.)
    // حُذف هذا السطر بصمت مرةً (نشرة v5.33 من شجرةٍ أقدم — seedRbac يعيد البذر من هنا):
    // من يزيله يُسقط اختبار bd-project-management.test.js — وهذا مقصود.
