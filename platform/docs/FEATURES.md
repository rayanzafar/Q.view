...
- **/app/opportunity/:id** — `opportunityDetailPage` (src/web/views/opportunity-detail.js): stage history, team, partner departments, commercial attributes; partner-department managers get the control card + team form, with attribution selects (sector/dept/partners) disabled-not-hidden (`canEditAttribution`, ADR-0006). Gate = opportunities page.
