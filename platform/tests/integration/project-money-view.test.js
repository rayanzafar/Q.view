...
const finance = U({ id: 'u_fin', username: 'fin1', name_ar: 'المالية', role_id: 'finance', scope: 'company' });
...
test('مدير المشروع يرى نِسَب التسكين ويقرأ سبب حجب أرقام المال', async () => {
  const m = moneyOf(await projectDetailPage(pm, 'P1'));
  assert.ok(m.includes('سارة العتيبي'), 'النِّسَب حق تشغيلي');
  assert.ok(m.includes('خارج صلاحيات دورك'), 'سبب الحجب مكتوب لا مسكوت عنه');
  assert.ok(!m.includes(fmtSar(120_000)), 'لا مبلغ تحصيل لمن لا يقرأ الفوترة');
