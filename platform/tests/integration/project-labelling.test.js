...
test('صفحة المشروع: «الإيراد المُثبت» يأتي من البنود — والرقم الشبح القديم لا يُعرض', async () => {
  ...
  // وما يحلّ محلّه ليس صفراً بل «غير مُسجَّل». كان الفحص يشترط «٠ ر.س.» بوصفه الدليل على أن
  // الرقم الموروث خرج، وهو دليلٌ صحيح لكنه يثبّت صياغةً تخالف قاعدة المنتج نفسها: الصفر رقمٌ
  // يقول «حُسِب فكان لا شيء»، والغياب يقول «لم يُدخله أحد بعد» — والفرق بينهما قرارٌ وعُطل.
  assert.match(ghost, /الإيراد المُثبت[\s\S]{0,220}?غير مُسجَّل/, 'الغياب يُكتب غياباً لا صفراً');
  assert.doesNotMatch(ghost, /الإيراد المُثبت[\s\S]{0,220}?\b0\sر\.س/, 'ولا يُطبع رقمٌ لم يقله أحد');
});
