...
| /api approvals (inline) | GET `/approvals/queue`, POST `/approvals`, POST `/approvals/:id/act`, GET `/approvals/pending-count` (live badge count: role queue + directed) | src/modules/workflow/engine.js | One box for role-threshold workflows + direct (assignee) approvals: staffing confirmations and task approvals |
