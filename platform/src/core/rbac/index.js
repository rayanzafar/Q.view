...
// ── الموارد التي تُبنى قوائمها بالقطاع في نطاق «الإدارة» ──────────────────────
// حصرٌ مقصود وضيّق — **الفرصة وحدها**، وهذا ما نصّ عليه حارسٌ قائم حرفاً: «التوسيع المطلوب
// كان على الفرص لا على الناس» (tests/security/department-manager-reach.test.js). فقائمة الفرص
// تُبنى بالقطاع في هذا النطاق (`listOpportunities` بلا deptCol)، ومنحُ مدير الإدارة عليها
// مقصودٌ أن يريه فرص قطاعه — فتوسعةُ قراءة الصفّ تلحق بها كي يتطابق ما يُعرَض بما يُفتح.
//
// وأول محاولةٍ عندي وسّعت **كل** مورد فأسقطت ثلاثة فحوص أمنية كانت على حق: انفتحت قراءة
// «الموظف» و«المشروع» على القطاع كله وهما محدودان بالإدارة عن قصد ومحروسان باثني عشر فحصاً،
// وبلغ مدير الإدارة إداراتٍ لا يقودها في شاشة منح الصلاحيات. والدرس أن التوسعة تلحق **بمورد
// بعينه اتّسعت قائمته وقُصد اتساعها**، لا بالنطاق كله.
//
// ويبقى المشروع بتناقضه: محفظته تُبنى بالقطاع وصفُّه محدودٌ بالإدارة. وعلاجه **تضييق القائمة**
// لا توسيع الصفّ — وهو مؤجَّل بقرارٍ موثَّق حتى تُنسَب المشاريع إلى إداراتها، إذ التضييق اليوم
// يُخفي كل مشروعٍ بلا إدارة (`departmentInSql` لا يشمل الفارغ) فيستبدل تسريباً بعُطل.
const SECTOR_WIDE_LISTS = new Set(['opportunity']);
  ...
  switch (scope) {
    ...
    case 'department': {
      ...
      if (target.department_id) {
        ...
        // ── وما يُعرَض في القائمة يجب أن يُفتح ────────────────────────────
        // بناءُ القوائم لهذا النطاق **يفشل مفتوحاً إلى القطاع** عن قصد موثَّق (scope.js): عمود
        // الإدارة لا يُمرَّر للاستعلامات قبل نسبة البيانات، فمدير الإدارة يقرأ في قوائمه كل صفوف
        // قطاعه. وهذا الفحص كان يقصّ على إداراته وحدها — فيرى الصفَّ في القائمة ثم يُرَدّ عنه
        // بـ«صلاحيتك لا تسمح» حين يضغطه. وقع ذلك على المالك نفسه: فرصةٌ في «المدن الذكية»
        // تظهر له في «الفرص» ولا تُفتح.
        //
        // والتناقض قديم، لكنه كان مستوراً: أكثر الصفوف بلا إدارة فتمرّ من الفرع أدناه. ولمّا
        // نُسبت الفرص إلى إداراتها ظهر على صفوفٍ حقيقية.
        //
        // فالقراءة تتبع القائمة — لا تفتح شيئاً جديداً، بل تُنهي كسراً في وصولٍ قائم فعلاً.
        // **والكتابة لا تتبعها**: تعديلُ فرصةِ إدارةٍ أخرى سلطةٌ لم تُمنَح، وتوسيعها هنا يحوّل
        // إصلاح عرضٍ إلى منح صلاحية. فمن رأى وأراد التعديل يُطلَب له المنح صراحةً.
        // وثلاثة شروط تلزم معاً: مورده من الموارد التي اتّسعت قوائمها · والفعل قراءة · وداخل
        // قطاعه هو. وإسقاطُ أيٍّ منها يفتح ما لم يُقصَد — وقد أسقطتُ الأول مرةً فأسقطتني الحزمة.
        return action === 'read' && !!resource && SECTOR_WIDE_LISTS.has(resource)
          && !!user.sector_id && target.sector_id === user.sector_id;
      }
