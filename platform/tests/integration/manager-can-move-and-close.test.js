...
// العطل الأصلي: النقل ينجح ثم يُرمى خطأٌ فيظنّه المالك فشل. وهذا ما يحرسه الفحص أولاً وأخيراً.
//
// ومنذ قلب الرؤية (قرار المالك ٢٠٢٦-٠٨: مدير الإدارة يرى فرص **إدارته** لا قطاعه) صار نقلُ
// الفرصة إلى إدارةٍ أخرى يُخرجها من نطاقه فعلاً — كما يُخرجها نقلُ القطاع من نطاق قائده.
// والحارس واحد في الحالين: الكتابة تُكتب، ويعود **تأكيدٌ صادق مختصر** (movedOutOfReach) بدل
// رسالة «صلاحيتك لا تسمح» الكاذبة على عملٍ نجح.
test('وينقلها إلى إدارة أخرى بلا رسالة منعٍ كاذبة — والنقل يُكتب فعلاً', async () => {
  ...
  assert.ok(res, 'لم يُعَد شيء للناقل');
  assert.equal(res.department_id, 'D_AI', 'والردّ يحمل الإدارة الجديدة');
  assert.equal(res.id, oid);
  assert.equal(res.movedOutOfReach, true, 'نقلها إلى إدارةٍ ليست له ثم قيل له «ممنوع» على عملٍ نجح');
  // وبعد النقل صارت لغيره: لا تُفتح له — وهذا هو الإسناد لا عطلاً فيه.
  await assert.rejects(() => opps.getOpportunity(DM, oid), (e) => e.status === 403);
});
