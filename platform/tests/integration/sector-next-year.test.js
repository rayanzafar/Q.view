...
test('السنة القادمة تظهر في المنتقي لوجود صفقةٍ مُسنَدة إليها — موسومةً «قادمة»', async () => {
  // القاعدة بعد قرار الإخفاء (2026-08-25): لا حقنَ لسنةٍ قادمة فارغة — لكن سنةً لها سجلّ
  // (هنا فرصة O_NEXT مُسنَدة إليها) تظهر كسائر سنوات البيانات، بوسمها.
  const h = await sectorPage(ADMIN, { year: String(YEAR) });
  assert.ok(h.includes(`سنة ${NEXT} — قادمة`), 'سنةٌ لها صفقة مُسنَدة غائبة عن المنتقي');
});

test('منتقي السنة في شريط المرشِّحات نفسه — وروابطه تحفظ الفصل والمرشِّح', async () => {
  // ملاحظة أ. حسين (2026-08-25): «لا أرى منتقي السنة في أي مكان» — كان في رأس الصفحة البعيد
  // وحده. صار مع الفترة والإدارة والعملاء، وروابطه تحمل بقية حالة الصفحة كسائر المرشِّحات.
  const h = await sectorPage(ADMIN, { year: String(YEAR), tab: 'com' });
  const tb = h.slice(h.indexOf('class="toolbar"'), h.indexOf('id="kpi-band"'));
  assert.ok(tb.includes('السنة:'), 'تسمية «السنة:» غائبة عن شريط المرشِّحات');
  const link = tb.match(new RegExp(`<a class="fitem[^"]*" href="([^"]+)"[^>]*>سنة ${NEXT} — قادمة</a>`));
  assert.ok(link, 'رابط السنة القادمة غائب من منتقي الشريط');
  assert.ok(link[1].includes(`year=${NEXT}`) && link[1].includes('tab=com'), 'رابط السنة لا يحفظ الفصل الحالي');
});
...
test('السنة القادمة: لا سجلّ يُختلق، والتوقع امتدادُ وتيرةٍ معلَن، والمجدول بجانبه', async () => {
  ...
  // وخلاصة الشريط لا تدّعي مساراً زمنياً ولا مقارنةً سنوية لسنةٍ لم تبدأ
  const band = h.slice(h.indexOf('class="exec-band"'), h.indexOf('role="tablist"'));
  assert.ok(band.includes('سنةٌ قادمة — لا سجلّ'), 'خلاصة الشريط لا تقول «سنة قادمة»');
  assert.ok(!band.includes('من المستهدف'), 'الشريط يدّعي نسبةً من مستهدفٍ لسنةٍ لم تبدأ');
  assert.ok(!band.includes('−100%'), 'الشريط يدّعي تراجعاً −100% لسنةٍ لم تبدأ');
});
