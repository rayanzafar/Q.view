...
// ── والمهمة الشخصية خارج كل عدٍّ وكل قائمة ────────────────────────────────────
// «داخلي» عملٌ للشركة بلا مشروع، ومديرك يقرؤه بحقّ. و«شخصية» لا تخرج من حساب صاحبها: لا في
// لوحة الفريق، ولا في ملف الشخص حين يفتحه غيره، ولا في تقارير الفترة، ولا في قوائم المساعد،
// ولا في مقياس الحِمل. ولو تسرّبت مرةً واحدة لبطل معناها كله ولن يكتب فيها أحدٌ شيئاً بعدها.
// فالحارس مصدرٌ واحد يستورده كل من يقرأ المهام عبر الحسابات، لا شرطٌ منسوخ في ستة مواضع
// ينسى سابعُها. ويشمل الصفوف القديمة التي عمودها فارغ (لا شيء منها شخصي). الشرحُ الكامل
// لمعنى «شخصية» في رأس `tasks.js` حيث وُلدت، وهي تُعاد تصديراً من هناك.
...
// و`myWorkOrMyPendingSql` لعضوية قائمة «مهامي» نفسها: ما أُسند إليّ، **وما كتبتُه أنا وما
// يزال ينتظر اعتماداً ولو باسم غيري** — من أسند مهمةً وذهبت للاعتماد يرى أين وقفت بدل أن
// تختفي حتى تُحسم فلا يعرف أوصلت أم ضاعت (قرار المالك، KI-041 سابقاً). وتُعرض معلَّمةً
// خارج العدّادات: قرارٌ معلَّق لا عملٌ قائم.
export const myWorkOrMyPendingSql = (pfx = 't.', userId = null) => ({
  clause: `(${pfx}assignee_user_id = ? OR (${pfx}created_by = ? AND ${pfx}approval_state = '${TASK_PENDING}'))`,
