...
| oppdocsRouter | GET/POST `/opportunities/:id/documents`, DELETE `/opportunities/documents/:docId`, POST `/opportunities/:id/documents/upload` (raw ≤15mb, `x-file-name`, ext allow-list → canonical mime), GET `/opportunities/documents/:docId/download` (attachment + nosniff) | src/modules/crm/oppdocs.js (oppdocs.routes.js) | Links stay metadata in shared `document`; uploaded bytes live in `document_blob` (033, ADR-0007) — delete purges bytes, soft-deletes metadata; both gated by opportunity read/update reach |
...
- **/app/opportunity/:id** — `opportunityDetailPage` (src/web/views/opportunity-detail.js): read-first (v5.24) — header stat strip + stage rail + 5 ARIA tabs (overview/activity/docs/team/history, initial from `?tab=`, all panels server-rendered); «التحكم بالفرصة» form lives verbatim in inert `<template id="opp-edit-tpl">` opened as a modal (also via `?edit=1`), team form in `<template id="opp-team-tpl">`; docs tab = real upload drop-zone (`document_blob`) + external links; team tab shows pending join requests (`pendingTeamApprovals`); history tab adds audit trail for admin only (same gate as the audit page); partner-department managers keep the control card with attribution selects disabled-not-hidden (`canEditAttribution`, ADR-0006). Gate = opportunities page.
