...
before(async () => {
  ...
  // الأولى **يملكها رئيس تطوير الأعمال** — فمن يظهر في ملفه مسكَّناً ليس صاحبها.
  // و«الابتكار» عليها عمداً: التسكين بوابته `update opportunity`، ومدير الإدارة يملكها
  // على فرص إدارته لا على كل فرصة. فهذه هي الحالة الحيّة: مديرٌ يضمّ أحد رجاله إلى فرصةٍ في
  // إدارته يقودها تطوير الأعمال. والثانية **يملكها مديرُ تطوير الأعمال نفسه** — فمنذ قلب
  // الرؤية (٢٠٢٦-٠٨) لا يحرّك BD إلا فرصه المملوكة — ويسكِّن عليها موظفاً لا يملك أمره،
  // فيلزم تأكيد مديره: مسار التعليق.
  for (const [id, title, dept, owner] of [
    ['O_TEAM', 'منصة الاركاب الذكي', 'D1', 'u_owner'],
    ['O_PEND', 'مكتب البيانات ومركز ذكاء الأعمال', 'D2', 'u_bd'],
  ]) {
    await db.insert('opportunity', { id, title_ar: title, sector_id: 'SOL', department_id: dept,
      stage_id: 'LEAD', client_id: 'CL', value_halalas: 10000000, owner_user_id: owner,
...
test('وتسكينٌ ينتظر تأكيد المدير يظهر معلَّماً ولا يُحتسب — فلا يعدّ الرقم عملاً لم يُقَرّ', async () => {
  // المسكِّن هنا مدير تطوير الأعمال: يملك تعديل **فرصته المملوكة** ولا يملك الموظف — فيلزم
  // تأكيد مديره. (ولا يصلح مدير النظام هنا: هو يملك الجميع، فتسكينه لا يُعلَّق أصلاً — وهو المقصود.)
