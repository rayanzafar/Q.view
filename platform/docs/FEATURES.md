...
| `sector` | مركز القطاع | Executive command center (v5.21 rebuild, signal→context→drill-down): deterministic insight strip (no LLM text), revenue/sales hero KPIs with goal rings + pace chips, minis (coverage/win-rate/projects/margin-gated), stage-filterable funnel (count⇄value, purple gradient, click-to-filter chip), curated «ما تغيّر» (category filters, max-5+expand), «يحتاج تدخلك» by impact, team capacity (donut + avatar spectrum, now/30/90, roster gated by `can(u,'read','employee')`), one health donut, performance-vs-plan bullet, top-5 clients + concentration signal. Aggregates sector-wide, named items behind `scopeFilter` (policy v5.2). Personal face `mySectorPage` unchanged | `can(u,'read','project') \|\| can(u,'read','opportunity')` | src/web/views/sector.js | pages/sector.js | live |
