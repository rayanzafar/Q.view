...
| src/modules/crm | Opportunity pipeline: CRUD, stage/sector moves, won↔project mirror, opp team, opp documents | opportunities.js, opp-project-sync.js, oppteam.js, oppdocs.js | tests/integration/opp-to-project.test.js, project-opportunity-mirror.test.js, opp-team*.test.js, opportunity-*.test.js, tests/unit/opp-rot.test.js |
...
- **/app/person/:id** — `personPage` (src/web/views/pmo.js): person dossier + action hub (staff, assign task, grants). **Deliberately no route guard** — `personDossier` (src/modules/pmo/tasks.js) authorizes per row; own page always opens. Page JS: pages/person.js.
