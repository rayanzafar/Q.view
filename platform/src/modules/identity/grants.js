...
export const GRANTABLE = [
  ...
  // كانت أزواج v5.60 خمسةً، فصارت أربعة. حذفُ الفعالية يمحو صور بطاقاتها ورموز كشكها محواً
  // فعلياً لا رجعة فيه، فبابه مدير النظام وحده (قرار حسين ٢٠٢٦-٠٩-٠١ — matrix.js وADR-0013).
  // **ولا ترحيلة ولا سكربت رفع**: الصفوف الثلاثة القائمة على الحيّ بـevent:delete تبطل بنفسها
  // لحظة النشر، لأن `grantsForUser()` أدناه يمرّر كل صفٍّ على `isGrantable()` ويُسقط ما ليس في
  // هذه القائمة. فلا يبحثنّ أحدٌ عن ترحيلةٍ ناقصة — الصفّ يبقى أثراً ولا أثر له في الصلاحية.
  {
