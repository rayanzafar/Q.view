...
// آخرُ بابٍ إلى صفّ المهمة الشخصية من حسابٍ آخر: تسجيل الوقت يقبل **أي معرّف مهمة** ثم يرحّل
// الساعات إليها. الأثر كتابةٌ لا قراءة، لكنه يخرق التعهّد نفسه — ومن يستطيع الكتابة على صفٍّ
// يستطيع أن يستدلّ على وجوده.
test('ولا تُرحَّل ساعاتُ أحدٍ إلى مهمة غيره الشخصية', async () => {
  const ts = await import('../../src/modules/timesheets/timesheets.js');
  await assert.rejects(() => ts.addEntry(CTX_BOSS,
    { hours: 2, entry_date: TODAY, task_id: PERSONAL_ID }),
  (e) => /غير موجودة/.test(e.message), 'سُجِّل وقتٌ على مهمة شخصية لشخص آخر');
  // وصاحبتها تسجّل وقتها عليها كما تسجّله على أي مهمة لها — الحارس شرطٌ لا تضييقٌ عام.
  await ts.addEntry(CTX_ME, { hours: 2, entry_date: TODAY, task_id: PERSONAL_ID });
  assert.equal((await db.get('SELECT actual_hours FROM task WHERE id = ?', [PERSONAL_ID])).actual_hours, 2);
});
...
// مكانَ الكتابة والقائمة الكاملة، لكن ما كُتب يُقرأ مع اليوم لا بعد نقرةٍ ثانية.
// ويُحرَس معه أن الملاحظة **لا تخرج من حساب صاحبها**: لوحة الفريق ليست مكانها.
test('الملاحظات تظهر في صفحة «مهامي» نفسها، ولا تظهر في لوحة الفريق', async () => {
  await notes.createNote(CTX_ME, { subject: 'محضر اجتماع الترسية', body: 'نقاط المتابعة مع الجهة' });

  const mine = await P.tasksPage(ME, {});
  assert.ok(mine.includes('ملاحظاتي'), 'لا مكان للملاحظات في صفحة مهامي');
  assert.ok(mine.includes('محضر اجتماع الترسية'), 'ما كُتب لا يظهر مع اليوم');

  // ولا تخرج من حساب صاحبها: مدير النظام يفتح لوحة الفريق فلا يجدها.
  const team = await P.tasksPage(BOSS, { who: 'team' });
  assert.ok(!team.includes('محضر اجتماع الترسية'), 'ملاحظةٌ شخصية تسرّبت إلى لوحة الفريق');
});
