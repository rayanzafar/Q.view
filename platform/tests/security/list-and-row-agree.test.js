...
test('قائمته فرصُ إدارتيه — مسؤولةً أو مشاركة — والأيتام، وكل صفٍّ فيها يُفتح فعلاً', async () => {
  const listed = await opps.listOpportunities(RAYAN, { year: 2026 });
  // إدارتاه (opp_mine) + ما تشارك فيه (opp_partner) + اليتيمة في قطاعه (opp_nodept) — لا فرصُ
  // إدارةٍ أخرى مُسنَدة. القائمة لم تَعُد قطاعيةً كاملة، لكنها تشمل ما لا إدارة له (v5.5).
  assert.deepEqual(listed.map((o) => o.id).sort(), ['opp_mine', 'opp_nodept', 'opp_partner'],
    'القائمة: إدارتاه + المشاركة + أيتام قطاعه — لا فرصَ إدارةٍ أخرى مُسنَدة');
...
// ── الفرصة «بلا إدارة» — القائمة تُحاذي الصفَّ (تصحيح v5.5) ───────────────────
// كان الصفُّ يُفتح بالعنوان المباشر داخل القطاع بينما القائمة تُخفيه — فرصةٌ تُفتح ولا تُعرَض.
// شكا منها مالكُ الأداة نفسه (ريان): «ما عاد يطلع لي كل الفرص… ومشاريع كانت WIN اختفت» —
// وأكثرُ الأيتام بورتفوليو فوزٍ تاريخيّ مستورد بلا مالكٍ ولا إدارة. فالآن القائمة تعرض الفرصة
// بلا إدارةٍ في قطاع مدير الإدارة كما تُفتح صفّياً: صفٌّ لم يُنسَب بعد ليس سرَّ إدارةٍ أخرى بل
// عملاً مشتركاً في القطاع (ومنه ما يخصّ أكثر من إدارة). لا كشفَ جديد — الصفُّ كان مفتوحاً.
test('فرصة بلا إدارة: تظهر في قائمة مدير إدارتها **وتُفتح** — القائمة تُحاذي الصفَّ', async () => {
  const listed = await opps.listOpportunities(RAYAN, { year: 2026 });
  assert.ok(listed.some((o) => o.id === 'opp_nodept'), 'صفٌّ بلا إدارةٍ في قطاعه غاب عن قائمته');
  const row = { id: 'opp_nodept', sector_id: 'SOLUTIONS', department_id: null };
  assert.equal(can(RAYAN, 'read', 'opportunity', row), true,
    'ويُقرأ صفّياً كما يُعرَض — القائمة والصفّ اتّفقا');
});
