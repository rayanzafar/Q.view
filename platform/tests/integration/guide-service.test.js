...
test('الترتيب: الشاشات تأتي بترتيب رحلة صاحبها لا بترتيب القائمة الجانبية', async () => {
  ...
  // مكتب الرئيس التنفيذي: بعد «صفحتي» تبدأ رحلته من لوحة القيادة. والفهرس واحدٌ لا صفر منذ
  // صارت «صفحتي» وجهة الدخول للجميع (كما في قائد القطاع أدناه).
  //
  // و«المالية والعقود» **لم تعد في دليل أحد** — أُزيلت الشاشة بقرار المالك، والدليل يمرّ من
  // بوابة الفتح نفسها فيسقط وصفُها تلقائياً. وهذا هو الحدّ المقصود: دليلٌ لا يصف شاشةً مغلقة،
  // فلا يَعِد الموظفَ بما لا يجده. (وسجلُّ الوقت سبقها إلى ذلك بالقرار نفسه.)
  ...
  assert.ok(!fin.includes('finance'), 'دليلٌ يصف شاشة مالية مُزالة — وعدٌ بما لا يُفتح');
  for (const role of ['admin', 'sector_lead', 'project_manager', 'employee']) {
    const keys = (await guide.guideFor(U(role))).pages.map((p) => p.key);
    assert.ok(!keys.includes('finance'), `${role}: «المالية» في دليله وهي مُزالة`);
    assert.ok(!keys.includes('timesheet'), `${role}: «سجل الوقت» في دليله وهو مُزال`);
  }
