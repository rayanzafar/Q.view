...
import { login, open, collectErrors, PAGES } from './_helpers.mjs';
...
export default async function rtlSpec({ browser, base, t }) {
  for (const username of ROLES) {
    for (const viewport of WIDTHS) {
      ...
      if (consoleErrors.length) { bad++; t.fail(`${username}@${viewport.width} console`, consoleErrors.slice(0, 3).join(' | ')); }
