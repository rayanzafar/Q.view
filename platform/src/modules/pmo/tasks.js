...
export function myTasks(user, filters = {}) {
  const where = ['deleted_at IS NULL', 'assignee_user_id = ?'];
  const params = [user.id];
  if (filters.status) { where.push('status = ?'); params.push(filters.status); }
  if (filters.today) { where.push("(due_date IS NULL OR date(due_date) <= date('now'))"); }
  return all(`SELECT * FROM task WHERE ${where.join(' AND ')} ORDER BY
    CASE priority WHEN 'P0' THEN 0 WHEN 'P1' THEN 1 WHEN 'P2' THEN 2 ELSE 3 END, due_date`, params);
}

export function projectTasks(user, projectId) {
  ...
  return all('SELECT * FROM task WHERE project_id = ? AND deleted_at IS NULL ORDER BY status, due_date', [projectId]);
}

// Quick Add — minimal fields, instant. Defaults assignee to self.
export function quickAddTask(ctx, data) {
  const user = ctx.user;
  ...
  audit(ctx, { action: 'create', resource: 'task', resourceId: tid, sectorId: data.sector_id || user.sector_id });
  return get('SELECT * FROM task WHERE id = ?', [tid]);
}

export function updateTask(ctx, taskId, data) {
  const user = ctx.user;
  const row = get('SELECT * FROM task WHERE id = ? AND deleted_at IS NULL', [taskId]);
  ...
  audit(ctx, { action: 'update', resource: 'task', resourceId: taskId, detail: { status: patch.status } });
  return get('SELECT * FROM task WHERE id = ?', [taskId]);
}
