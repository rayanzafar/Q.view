...
//
// وتبدّل شيءٌ في الطريق: نقلُ الفرصة إلى إدارةٍ أخرى **داخل قطاعه** لم يعد يُخرجها من قراءته —
// قراءةُ الفرص تتبع القائمة الآن (القائمة تُبنى بالقطاع في هذا النطاق، فيرى فرص قطاعه كلها).
// فالفرصة تبقى مقروءةً بعد النقل ويُعاد صفُّها كاملاً بدل التأكيد المختصر.
// والحارس نفسه باقٍ حيث تخرج الفرصة فعلاً — نقلُ القطاع، ويغطّيه الفحص التالي.
test('وينقلها إلى إدارة أخرى بلا رسالة منعٍ كاذبة — والنقل يُكتب فعلاً', async () => {
  ...
  // ولا يزال يفتحها بعد النقل — فلا شاشة تُعاد إلى صفحةٍ يردّها النظام.
  await assert.doesNotReject(() => opps.getOpportunity(DM, oid));
});
