...
// ── روابط الاستقبال العامة (مركز التطوير) ────────────────────────────────────────────────
// هذه المسارات وحدها في المنصة يبلغها من لا حساب له — فلا كعكةَ جلسةٍ تحدّه ولا حسابَ يُقفل.
// العنوان هو كل ما نملك، والدلو هو الحدّ الوحيد.
//
// الإرسال: خمسةُ بلاغاتٍ ثم قطرةٌ كل ٢٤ ثانية (أي خمسةٌ كل دقيقتين بعد النفاد). العدد مقصود:
// من يبلّغ عن عُطلٍ حقيقي قد يرسل بلاغين أو ثلاثة في جلسةٍ واحدة — صورةٌ نسيها، أو تفصيلٌ
// تذكّره — فلا يُردّ. ومن يرسل عشرات في الدقيقة ليس مُبلِّغاً. والحقلُ الخفيّ (honeypot) يمسك
// الآلة الغبية، وهذا يمسك المُصرّ. والردّ حمولةٌ عربية لا تحويلة: النموذج يُرسَل من نصٍّ
// برمجي في الصفحة نفسها فيقرأ الرسالة ويعرضها في مكانها، ولا صفحةَ دخولٍ يُعاد إليها أصلاً.
export const publicFormLimiter = bucketLimiter({ capacity: 5, refillPerSec: 1 / 24, keyFn: (req) => `P:${req.ip}` });
// وفتحُ الصفحة نفسها أوسع بكثير: قراءةٌ لا كتابة، وقد يفتحها فريقُ عميلٍ كامل من عنوانٍ واحد
// في اليوم نفسه. السقف يمنع من يستكشف الروابط بالتخمين آلياً، ولا يزعج من يقرأ.
export const publicPageLimiter = bucketLimiter({ capacity: 60, refillPerSec: 1, keyFn: (req) => `PP:${req.ip}` });
