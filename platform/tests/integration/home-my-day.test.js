...
test('لكل يومٍ عبارته الثابتة — والجمعة والسبت عطلة', () => {
  ...
  assert.equal(at('2026-08-12').hi, at('2026-08-19').hi, 'عبارة اليوم تتغيّر بين أربعاءين');
  assert.notEqual(at('2026-08-12').hi, at('2026-08-13').hi, 'يومان مختلفان بعبارةٍ واحدة');
  for (let i = 0; i < 7; i++) {
    const g = home.greetingFor(new Date(Date.UTC(2026, 7, 9 + i, 9)));
    assert.ok(g.hi && g.sub && g.weekday, 'يومٌ بلا عبارة كاملة');
  }
});
