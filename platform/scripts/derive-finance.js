...
export function deriveFinance() {
  const invoices = all('SELECT * FROM invoice WHERE deleted_at IS NULL');
  let owners = 0, coll = 0, dated = 0;
  for (const inv of invoices) {
    const patch = {};
    if (!inv.owner_user_id && inv.project_id) {
      const p = get('SELECT owner_user_id FROM project WHERE id=?', [inv.project_id]);
      if (p?.owner_user_id) { patch.owner_user_id = p.owner_user_id; owners++; }
    }
    if (!inv.issue_date) {
      const d = inv.deliverable_id ? get('SELECT delivered_at, month, year FROM deliverable WHERE id=?', [inv.deliverable_id]) : null;
    ...
    if (Object.keys(patch).length) run(`UPDATE invoice SET ${Object.keys(patch).map((k) => k + '=?').join(',')} WHERE id=?`, [...Object.values(patch), inv.id]);
    // PAID invoice with no collection → create a collection (settles AR)
    if (String(inv.status).toUpperCase() === 'PAID') {
      const has = get('SELECT id FROM collection WHERE invoice_id=?', [inv.id]);
      if (!has) {
        insert('collection', { id: id('col'), invoice_id: inv.id, amount_halalas: inv.amount_halalas,
      ...
      run("UPDATE invoice SET status='ISSUED' WHERE id=?", [inv.id]);
    }
  }
...
if (import.meta.url === `file://${process.argv[1]}`) deriveFinance();
