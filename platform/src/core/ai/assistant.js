...
export async function ask(ctx, message, opts = {}) {
  ...
  switch (intent) {
    ...
    case 'propose_change':    return await proposeChange(ctx, text, opts);
...
async function summarizeProject(user, projectId) {
  ...
  const p = await get('SELECT * FROM project WHERE id = ? OR code = ?', [projectId, projectId]);
  ...
  return await llmOrLocal(user, `لخّص حالة المشروع «${p.name_ar}» بإيجاز تنفيذي وبنبرة مهنية.`, facts,
    () => `**${p.name_ar}** — ${facts.join(' · ')}.` +
      (p.rag === 'RED' ? ' ⚠ المشروع حرج ويحتاج تدخلًا.' : k.lateTasks > 0 ? ' ⚠ توجد مهام متأخرة.' : ' على المسار.'));
}

async function weeklyReportDraft(ctx) {
  const data = await buildReport('weekly_exec_brief', ctx.user, {});
  const facts = [
  ...
  return await llmOrLocal(ctx.user, 'اكتب مسودة الموجز التنفيذي الأسبوعي بالعربية بنبرة تنفيذية موجزة.', facts,
    () => `**مسودة الموجز التنفيذي الأسبوعي**\n\n${facts.map((f) => '• ' + f).join('\n')}\n\n(يمكنك إرسالها من «التقارير والبريد».)`);
}

async function detectRisks(user) {
  const red = await all("SELECT name_ar FROM project WHERE rag='RED' AND deleted_at IS NULL " +
    (user.scope === 'company' ? '' : 'AND sector_id = ?') + ' LIMIT 10', user.scope === 'company' ? [] : [user.sector_id]);
  const overdue = await all("SELECT p.name_ar, COUNT(*) n FROM task t JOIN project p ON p.id = t.project_id " +
    "WHERE t.status != 'DONE' AND t.due_date IS NOT NULL AND substr(t.due_date,1,10) < ? AND t.deleted_at IS NULL " +
    (user.scope === 'company' ? '' : 'AND p.sector_id = ?') + ' GROUP BY p.id ORDER BY n DESC LIMIT 5',
    user.scope === 'company' ? [nowIso().slice(0, 10)] : [nowIso().slice(0, 10), user.sector_id]);
  const lines = [];
...
async function dataQualityScan(user) {
  ...
  const noType = (await get('SELECT COUNT(*) n FROM client WHERE (type IS NULL OR type = \'\') AND deleted_at IS NULL')).n;
  const zeroPrice = (await get("SELECT COUNT(*) n FROM service_package WHERE price_halalas = 0")).n;
  const noOwner = (await get('SELECT COUNT(*) n FROM opportunity WHERE owner_user_id IS NULL AND deleted_at IS NULL ' + (co ? '' : 'AND sector_id = ?'), co ? [] : [sec])).n;
  const noDates = (await get('SELECT COUNT(*) n FROM project WHERE (start_date IS NULL OR end_date IS NULL) AND deleted_at IS NULL ' + (co ? '' : 'AND sector_id = ?'), co ? [] : [sec])).n;
  const items = [
...
async function suggestPriorities(user) {
  const tasks = await all(`SELECT title, priority, due_date FROM task WHERE assignee_user_id = ? AND status != 'DONE' AND deleted_at IS NULL
...
async function proposeChange(ctx, text, opts) {
  const user = ctx.user;
  ...
  if (opts.change?.type === 'opp_stage' || oppMatch) {
    ...
    const opp = await get('SELECT * FROM opportunity WHERE id = ? OR code = ?', [oppId, oppId]);
...
export async function applyChange(ctx, token) {
  ...
  if (p.type === 'opp_stage') {
    const opp = await get('SELECT * FROM opportunity WHERE id = ?', [p.oppId]);
    if (!can(user, 'update', 'opportunity', opp)) throw forbidden(); // re-check at apply time
    const st = await get('SELECT * FROM stage WHERE id = ?', [p.to]);
    await update('opportunity', p.oppId, { stage_id: p.to, win_pct: st?.default_win_pct ?? opp.win_pct,
      stage_changed_at: nowIso(), updated_at: nowIso(), updated_by: user.id });
    await insert('opportunity_stage_history', { id: id('osh'), opportunity_id: p.oppId, from_stage_id: p.from,
      to_stage_id: p.to, changed_by: user.id, changed_at: nowIso(), note: 'AI-assisted (بتأكيد المستخدم)' });
    await audit(ctx, { action: 'update', resource: 'opportunity', resourceId: p.oppId, sectorId: opp.sector_id,
      detail: { via: 'ai', stage: `${p.from}→${p.to}` } });
    await logAsk(user, p.summary, 'propose_change', 1, p);
