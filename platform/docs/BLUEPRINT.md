...
الحالة: v5.75–v5.80 منشورة على staging ومتحقَّق منها حياً (2026-09-06؛ v5.79 عند `45a28d4`، v5.80 عند `184a19c`)، وv5.81 (ربط المساعد الذكي) منشورة ومتحقَّق منها حياً (2026-09-08 عند `faae1f3`). الأساس السابق: `1efbea3`، 2026-09-05.
...
ومراجعة البيانات ونمط SSR والاختبارات تكفي لهذه الدفعة. وMCP المنتج نُفِّذ في v5.81 على
القاعدة نفسها التي وُصفت هنا: خدمات محددة الهدف، تفويض المستخدم نفسه، قراءة أولًا، ثم
معاينة كتابة وسجل تدقيق. لا استعلام حر ولا حساب أدمن مشترك للمساعدين (ADR-0019).
...
| مساعد GPT/Claude بصلاحيات المستخدم | [MCP Authorization](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization) | نُفِّذ في v5.81: رمز لهذا الخادم وحده مربوط بموظف واحد، وفحص الجمهور عند الإذن، وبلا تمرير رمز طرف آخر ولا مشاركة حساب الأدمن. المتبقي: تجربة عميل خارجي حقيقي تُوثَّق بنتيجتها |
...
## ما بعد الحزمة — v5.79 وv5.80 (منشورتان على staging، 2026-09-06)

- **v5.79 (استلام ونشر):** الحزمة استُوردت بتقديمٍ سريع فوق `1efbea3` وتحقَّقت هنا (الاختبارات كاملة،
  PostgreSQL محلي، e2e، تحقق بصري)، ثم نُشرت بالمسار الواحد. خطُّ النشر صار يأخذ النسخة الاحتياطية عبر
  مسار التطبيق حين لا يبلغ pg_dump القاعدة، ويطبع جرد الترحيلات من النسخة قبل الرفع (043 كانت
  المعلَّقة الوحيدة، إضافية)، ووسم النشرة يُعلن في `/ready` (أُغلق KI-101). وسيلة الاستعادة:
  `scripts/restore-dump.mjs` فوق قاعدة مُرحَّلة.
- **v5.80 (KI-112 وKI-110):** التراجع المراجَع عن الفوز من نافذة نقل المرحلة نفسها (إبقاء المشروع
  مستقلاً بفرصة خاصة غير مؤكدة السنة، أو إلغاؤه بسجله) بسببٍ وأثرٍ على الطرفين ولا حذف؛ والتوزيع
  الدوري لمستهدف القطاع (12 شهراً بمجموعٍ يساوي السنوي حرفاً، سبب وإصدار وأثر) تقرأه صفحة القطاع
  بدل أي قسمة متساوية مفترضة. المتبقي من KI-110 قرارٌ مع المالك (تجميد إصدار التوزيع للتقارير الماضية).
- **v5.81 (ربط المساعد الذكي):** خادم MCP داخل سند بمصادقة OAuth 2.1 مربوطة بحساب الموظف نفسه، وصفحة «ربط المساعد» وشاشة إذنٍ عربية، وثلاث أدوات تعرّف المساعد بالمنصة من خدمة الدليل. أُغلق KI-097. تحقَّقت الرحلة كاملة حياً على بيئة التجربة (21/21) بعميلٍ يمشي المسار القياسي. المتبقي: تجربة المالك من مساعده هو، وقرار KI-113 (نافذة إدارية وعمرٌ أقصى للمنحة).
- **ما ينتظر المالك:** تقرير تسوية السجلات (KI-102/103/104) قراءةً من نسخة staging بلا تغيير — 171 سطر
  إيراد 2026 يحتاج توثيق فترة، 3 غير مربوطة بمخرج، موظف واحد باسمٍ يوحي بالتجريب، 11 فرصة بلا سنة بيع
  مؤكدة، ولا مستهدفات قطاعية مسجلة لسنة 2026 (القيم القديمة بلا سنة تنتظر التحديد). KI-109 قرار
  المالك (يُبقى مع التنبيه). لا تصحيح لسجل حي إلا بقراره.

## تنفيذ الفريق — v5.78 (منشور ضمن v5.79)
