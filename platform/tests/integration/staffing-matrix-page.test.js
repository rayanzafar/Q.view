...
test('القراءة للجميع: viewer يرى المصفوفة و#staff-q بلا أزرار كتابة', async () => {
  const viewer = { id: 'u_v', username: 'v', role_id: 'viewer', scope: 'sector', sector_id: 'S1' };
  const html = await P.staffingPage(viewer, {});
  assert.ok(html.includes('id="staff-q"'), 'مرساة الجولة يجب أن تُرندر لكل دور');
