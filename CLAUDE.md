...
2. **Staging deploys** go to Railway via `railway up` from `platform/` (staging.os.evcsol.com). Production releases happen only when the owner explicitly says so. Any `railway` command or staging-URL access is a **release action**: the pre-guard hook denies it unless the session sets `export SANAD_RELEASE=1`. Exploratory QA runs on a disposable local instance (`scripts/qa-up.mjs` / `/qa-explore`), never the live deployment.
